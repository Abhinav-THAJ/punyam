...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 2. Fetch Kundli Data
    const kundliUrl = new URL('https://api.prokerala.com/v2/astrology/kundli');
    kundliUrl.searchParams.append('datetime', datetime);
    kundliUrl.searchParams.append('coordinates', coordinates);
    kundliUrl.searchParams.append('ayanamsa', ayanamsa);
    ...
    if (!kundliRes.ok) {
      throw new Error(kundliData.errors?.[0]?.detail || 'Failed to fetch kundli');
    }
